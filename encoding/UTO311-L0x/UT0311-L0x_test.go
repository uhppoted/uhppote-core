...
func TestUnmarshalAs(t *testing.T) {
	...
	v, err := UnmarshalAs(message, reply)
	if err != nil {
		t.Fatalf("Unexpected error: %v", err)
	} else if v == nil {
		t.Fatalf("Expected valid reply, got: %v", v)
	}

	if reply.MsgType != 0x12 {
		t.Errorf("Expected 'byte':0x%02X, got: 0x%02X\n", 0x12, reply.MsgType)
	}

	if reply.Uint32 != 345678 {
		t.Errorf("Expected 'uint32':%d, got: %v\n", uint32(345678), reply.Uint32)
	}

	if v.(test).MsgType != 0x94 {
		t.Errorf("Expected 'byte':0x%02X, got: 0x%02X\n", 0x94, v.(test).MsgType)
	}

	if v.(test).Uint32 != 2018915346 {
		t.Errorf("Expected 'uint32':%d, got: %v\n", uint32(2018915346), v.(test).Uint32)
	}
}

func TestUnmarshalAsWithPtr(t *testing.T) {
	type test struct {
		MsgType types.MsgType `uhppote:"value:0x94"`
		Uint32  uint32        `uhppote:"offset:4"`
	}
...
func TestUnmarshalArrayElement(t *testing.T) {
	type test struct {
		MsgType types.MsgType `uhppote:"value:0x94"`
		Uint32  uint32        `uhppote:"offset:4"`
	}
