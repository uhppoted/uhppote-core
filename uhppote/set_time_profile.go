...
func (u *UHPPOTE) SetTimeProfile(deviceID uint32, profile types.TimeProfile) (bool, error) {
	...
	request := messages.SetTimeProfileRequest{
		...
		From:            *profile.From,
		To:              *profile.To,
		Monday:          profile.Weekdays.Monday,
		Tuesday:         profile.Weekdays.Tuesday,
		Wednesday:       profile.Weekdays.Wednesday,
		Thursday:        profile.Weekdays.Thursday,
		Friday:          profile.Weekdays.Friday,
		Saturday:        profile.Weekdays.Saturday,
		Sunday:          profile.Weekdays.Sunday,
