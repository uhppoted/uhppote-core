...
func TestMarshalSetAntiPassbackRequest(t *testing.T) {
	expected := []byte{
		0x17, 0x84, 0x00, 0x00, 0x78, 0x37, 0x2a, 0x18, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
func TestUnmarshalSetAntiPassbackResponse(t *testing.T) {
	message := []byte{
	...
	expected := SetAntiPassbackResponse{
		MsgType:      0x84,
		SerialNumber: 405419896,
		Ok:           true,
	}

	reply := SetAntiPassbackResponse{}
...
func TestFactoryUnmarshalSetAntiPassbackResponse(t *testing.T) {
	message := []byte{
		0x17, 0x84, 0x00, 0x00, 0x78, 0x37, 0x2a, 0x18, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	...
	} else if reply, ok := response.(*SetAntiPassbackResponse); !ok {
		t.Fatalf("invalid response type - expected:%T, got: %T\n", &SetAntiPassbackResponse{}, response)
	} else if reply == nil {
		t.Fatalf("invalid reply - expected:%T, got: %v\n", &SetAntiPassbackResponse{}, reply)
	} else if !reflect.DeepEqual(*reply, expected) {
		t.Errorf("incorrectly unmarshalled response\n   expected:%v\n   got:     \n%v", expected, reply)
	}
}

func TestUnmarshalSetAntiPassbackResponseWithInvalidMsgType(t *testing.T) {
	message := []byte{
